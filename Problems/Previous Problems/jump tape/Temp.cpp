...
int fun(int pos,int g)
{
    if(pos==n){
        if(g==1) return 0;
        else return INT_MAX;
    }
    if(mp[pos].find(g)!=mp[pos].end()) return mp[pos][g];
    int ret=0;
    ret=min( fun(pos+1,g)      , ar[pos].S  + fun(pos+1,__gcd(g,ar[pos].F))     );
    return mp[pos][g]=ret;
}
...
int32_t main()
{
    ...
    while(t--)
    {   
        //first mei jump_length , second mei cost
        cin>>n;
        int g=0;
        rep(i,n) cin>>ar[i].F,g=__gcd(g,ar[i].F);
        rep(i,n) cin>>ar[i].S;
